...
import carAnimeData from './carAnimeData';
...
export default class Car {
  ...
  constructor(el: CarParam) {
    const car = document.createElement('li');
    car.classList.add('car__box');
    car.id = `car__${el.id}`;
    car.innerHTML = `
      <div class="car__control control">
        <h3 class="car__name" id="car_name__${el.id}">${el.name}</h3>
        <div class="update__data unactive">
          <input type="text" minlength="1" required class="options__text car_name_update_btn" id="car_name_input__${el.id}" value="${el.name}">
          <input type="color" value="${el.color}" class="options__color" id="car_color_input__${el.id}">
        </div>
        <div class="control__col">
          <button class="btn control__start btn-control" id="car_start__${el.id}"><img src="./images/3279063_play_start_act_perform_show_icon.svg" alt="start" width="30px" height="30px"></button>
          <button class="btn control__stop btn-control btn-block" id="car_stop__${el.id}"><img src="./images/3279062_stop_avoid_cease_end_quit_icon.svg" alt="stop" width="30px"></button>
        </div>
        <div class="control__col">
          <button class="btn control__edit btn-control" id="car_edit__${el.id}"><img src="./images/8674943_ic_fluent_edit_settings_regular_icon.svg" alt="edit" width="30px"></button>
          <button class="btn control__remove  btn-control" id="car_remove__${el.id}"><img src="./images/3844424_can_delete_remove_trash_icon.svg" alt="remove" width="30px"></button>
        </div>
      </div>
      <div class="car__track">
        <svg class="car" xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 1000 600" overflow="visible" version="1.1"
    ...
    this.box = car;
    this.id = el.id;
    this.car = car.querySelector('.car') as HTMLElement;
    this.color = el.color;
    this.name = el.name;
    this.removeBtn = car.querySelector(`#car_remove__${el.id}`) as HTMLElement;
    this.editBtn = car.querySelector(`#car_edit__${el.id}`) as HTMLElement;
    this.colorInput = car.querySelector(
      `#car_color_input__${el.id}`,
    ) as HTMLInputElement;
    this.nameInput = car.querySelector(
      `#car_name_input__${el.id}`,
    ) as HTMLInputElement;
    this.startBtn = car.querySelector(`#car_start__${el.id}`) as HTMLElement;
    this.stopBtn = car.querySelector(`#car_stop__${el.id}`) as HTMLElement;
  }

  public createDriveAmination(duration: number) {
  ...
  public remove(callback: (arg0: number) => void) {
    this.removeBtn.addEventListener('click', () => callback(this.id));
  }

  public update(
    callback: (arg0: { name: string; color: string }, arg1: number) => void,
  ) {
    this.editBtn.addEventListener('click', () => callback(
      { name: this.nameInput.value, color: this.colorInput.value },
      this.id,
    ));
  }

  public drive(callback: (arg: Car) => void) {
    this.startBtn.addEventListener('click', () => callback(this));
  }
}
